import { BinaryTreeNode } from './NodeClass.js'
...
export function drawNode(value, canvasElement, x, y) {
  ...
  context.fillStyle = '#4caf50'; // Changed fill color to green
  context.fill();

  // Draw circle border
  context.beginPath();
  context.arc(x, y, DEFAULT_CONFIG.radius, 0, 2 * Math.PI, false);
  context.strokeStyle = '#2e7d32'; // Changed border color to a darker green
  context.lineWidth = 2; // Added line width for better visibility
  context.stroke();

  // Write value
  context.font = `${DEFAULT_CONFIG.fontSize}pt Arial`; // Changed font to Arial
  context.fillStyle = '#ffffff'; // Changed text color to white for better contrast
